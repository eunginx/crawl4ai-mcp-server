...
knowledge_graphs_path = parent_path / 'knowledge_graphs'
...
async def check_ai_script_hallucinations(ctx: Context, script_path: str) -> str:
    ...
    try:
        ...
        # Get the knowledge validator from lifespan context
        lifespan = ctx.request_context.lifespan_context
        knowledge_validator = lifespan.knowledge_validator
...
async def query_knowledge_graph(ctx: Context, command: str) -> str:
    ...
    **Node Types:**
    ...
    try:
        ...
        # Get Neo4j driver from lifespan context
        lifespan = ctx.request_context.lifespan_context
        repo_extractor = lifespan.repo_extractor
...
async def parse_github_repository(ctx: Context, repo_url: str) -> str:
    ...
    try:
        ...
        # Get the repository extractor from lifespan context
        lifespan = ctx.request_context.lifespan_context
        repo_extractor = lifespan.repo_extractor
